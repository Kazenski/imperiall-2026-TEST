...
import { collection, getDocs, doc, setDoc, updateDoc, deleteDoc, serverTimestamp, query } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
// Configuração para a instância secundária de criação de contas (evita deslogar o admin atual)
...
const secApp = initializeApp(firebaseConfig, "SecondaryAdminUsers");
...
window.userAdminTools = {
    updateRole: async function (uid, newRole) {
        try {
            ...
            // Recarrega de forma leve para atualizar as cores visuais do seletor
            loadUsersList();
        } catch (err) {
    ...
    executeBulkRegister: async function () {
        ...
        logArea.innerHTML = `<div class="text-purple-400 font-bold mb-1">> Iniciando processamento de lote...</div>`;
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            try {
                // Cria o usuário na instância secundária do Firebase Authentication
                const credential = await createUserWithEmailAndPassword(secAuth, email, password);
                const newUid = credential.user.uid;

                // Salva a estrutura de controle correta na coleção rpg_users do Firestore principal
                ...
                // Desconecta imediatamente o usuário recém-criado na instância secundária
