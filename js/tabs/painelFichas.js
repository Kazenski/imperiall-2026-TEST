...
// --- ABA 1: PAINEL DE FICHAS (LISTAGEM) ---
export function renderPainelFichas() {
    ...
    const adminHTML = globalState.isAdmin ? `
        <div class="bg-slate-800/80 p-4 rounded-xl border border-slate-700 mb-6 flex gap-4 items-center shadow-sm">
            <div class="w-12 h-12 rounded-full bg-indigo-900/50 flex items-center justify-center border border-indigo-500/30 shrink-0">
                <i class="fas fa-crown text-indigo-400 text-xl"></i>
            </div>
            <div class="flex-grow">
                <h3 class="text-sm font-bold text-slate-300 uppercase tracking-widest mb-1">Painel do Mestre</h3>
                <input type="text" id="admin-busca" placeholder="Buscar ficha de qualquer jogador..." class="w-full bg-slate-900 border border-slate-600 rounded text-slate-200 px-4 py-2 text-sm focus:border-amber-500 outline-none transition-colors">
            </div>
        </div>` : '';
    ...
    container.innerHTML = `
        <div class="w-full h-full flex flex-col animate-fade-in">
            <div class="flex justify-between items-end mb-6 border-b border-slate-700 pb-4">
                <div>
                    <h2 class="font-cinzel text-3xl text-amber-500">Compêndio de Aventureiros</h2>
            ...
            <div id="character-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 overflow-y-auto custom-scroll pr-2 pb-10">
...
function renderListaPersonagens(container, filtro) {
    ...
    filtered.sort((a,b)=>(a.nome||'').localeCompare(b.nome)).forEach(data => {
        ...
        item.className = 'bg-slate-800/80 p-4 rounded-xl border border-slate-700 hover:border-amber-500 hover:bg-slate-800 hover:shadow-[0_4px_20px_rgba(245,158,11,0.15)] cursor-pointer transition-all flex flex-col relative group h-40';
        ...
        item.innerHTML = `
            ...
            <div class="flex items-center gap-4 mb-3 relative z-0">
                <div class="w-16 h-16 rounded-full border-2 border-slate-600 group-hover:border-amber-400 overflow-hidden bg-black shrink-0 transition-colors">
...
// --- ABA 2: EDITOR DE FICHA (FULL SCREEN OPTIMIZED) ---
async function renderFichaEditor(fichaId) {
    ...
    container.innerHTML = `
        <div class="w-full h-full flex flex-col gap-4 animate-fade-in">
            
            <div class="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-sm shrink-0">
                <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Subclasse (Mestre)</label>
                        <select id="editor-subclasseId" class="${inputClass} appearance-none cursor-pointer"></select>
                    </div>
                </div>
            </div>

            <div class="grid grid-cols-4 md:grid-cols-8 gap-3 shrink-0">
                <div class="col-span-2 md:col-span-2 bg-slate-800 p-2 rounded-xl border border-slate-700 flex flex-col justify-center">
                    <label class="text-[9px] text-amber-500 font-bold uppercase tracking-widest px-2 mb-1">XP Total e Evolução</label>
                    <div class="flex items-center gap-2 px-2">
                        <input type="number" id="editor-experiencia" class="w-full bg-slate-900 border border-amber-900/50 rounded px-2 py-1 text-sm text-center font-bold text-amber-400 outline-none focus:border-amber-500">
                        <div class="w-full max-w-[120px] relative h-6 bg-black rounded border border-slate-600 overflow-hidden" id="xp-bar-container">
                            ...
                            <div id="xp-bar-text" class="absolute inset-0 flex items-center justify-center text-[9px] font-bold text-white text-shadow-sm">0/0</div>
                        </div>
                    </div>
                </div>
                
                <div class="col-span-2 md:col-span-1 bg-slate-800 p-2 rounded-xl border border-purple-900/30 flex flex-col justify-center text-center">
                    <label class="text-[9px] text-purple-400 font-bold uppercase tracking-widest mb-1">Pts. Extra</label>
                    <input type="number" id="editor-pontosExtrasMestre" class="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-sm text-center font-bold text-purple-300 outline-none" placeholder="0">
                </div>

                <div class="bg-slate-900 border border-slate-800 rounded-xl p-2 text-center shadow-inner flex flex-col justify-center">
                    <label class="text-[9px] text-sky-500 font-bold uppercase tracking-wider mb-0.5">Nível</label>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-2 text-center shadow-inner flex flex-col justify-center">
                    <label class="text-[9px] text-emerald-500 font-bold uppercase tracking-wider mb-0.5">HP Max</label>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-2 text-center shadow-inner flex flex-col justify-center">
                    <label class="text-[9px] text-blue-500 font-bold uppercase tracking-wider mb-0.5">MP Max</label>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-2 text-center shadow-inner flex flex-col justify-center">
                    <label class="text-[9px] text-amber-500 font-bold uppercase tracking-wider mb-0.5">Ini.</label>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-2 text-center shadow-inner flex flex-col justify-center">
                    <label class="text-[9px] text-purple-500 font-bold uppercase tracking-wider mb-0.5">Mov.</label>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 shrink-0">
                ...
                <div class="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-sm flex flex-col">
                    <div class="flex justify-between items-center mb-3 border-b border-slate-700 pb-2">
                        <span class="text-xs text-slate-400 uppercase tracking-widest font-bold"><i class="fas fa-hammer text-amber-500 mr-1"></i> Distribuição de Atributos</span>
                        <div class="flex items-center gap-2 bg-slate-900 px-3 py-1 rounded-full border border-slate-600">
                            <span class="text-[10px] text-slate-500 uppercase font-bold">Disponível:</span>
                            <div id="pontos-disponiveis" class="text-xl font-bold text-emerald-400 font-cinzel leading-none">0</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3 flex-grow">
                        ...
                        <div class="bg-slate-900/80 rounded-lg p-2 border border-slate-700 flex flex-col shadow-inner">
                            <h4 class="font-bold text-slate-400 text-center text-[10px] uppercase tracking-widest mb-1">${stat}</h4>
                            <div class="bg-black/40 rounded py-1 mb-2 text-center border border-slate-800">
                                <div id="display-total-${stat}" class="text-2xl font-bold text-amber-400 font-cinzel drop-shadow-sm">0</div>
                            </div>
                            
                            <div class="flex justify-between items-center bg-slate-800 px-1 rounded py-1 mb-2 border border-slate-600 select-none">
                                <button id="btn-${stat}-menos" class="w-6 h-6 rounded bg-slate-700 hover:bg-slate-600 text-slate-400 hover:text-white flex items-center justify-center font-bold shadow transition active:scale-95" title="-1 (Shift: -10, Ctrl: Máx)">-</button>
                                <span id="display-${stat}" class="font-bold text-white text-sm mx-1 text-center min-w-[20px]">0</span>
                                <button id="btn-${stat}-mais" class="w-6 h-6 rounded bg-amber-600 hover:bg-amber-500 text-white flex items-center justify-center font-bold shadow transition active:scale-95" title="+1 (Shift: +10, Ctrl: Máx)">+</button>
                            </div>

                            <div class="space-y-1 text-[9px] text-slate-500 font-mono px-1 mt-auto">
                ...
                <div class="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-sm flex gap-4 h-[200px]">
                    <div class="w-1/3 max-w-[150px] relative rounded-lg border border-slate-600 bg-black overflow-hidden shadow-inner group shrink-0">
                        <div id="main-image-placeholder-icon" class="absolute inset-0 flex flex-col items-center justify-center text-slate-600 ${fichaData.imagemPrincipal ? 'hidden' : ''}">
                            <i class="fas fa-image text-3xl mb-1 opacity-50"></i>
                            <span class="text-[9px] uppercase font-bold opacity-50 text-center px-2">Ficha sem Avatar</span>
                        </div>
                    ...
                    <div class="flex-grow flex flex-col">
                        <h4 class="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-2 flex items-center gap-2"><i class="fas fa-images text-sky-400"></i> Galeria de Aparências</h4>
                        <div id="image-upload-grid" class="grid grid-cols-4 sm:grid-cols-6 gap-2 overflow-y-auto custom-scroll pr-1 flex-grow"></div>
                    </div>
                </div>

            </div>

            <div class="flex flex-col flex-grow min-h-[300px] bg-slate-800 rounded-xl border border-slate-700 overflow-hidden shadow-sm">
                <div class="flex bg-slate-900 border-b border-slate-700 shrink-0">
                    <button class="bottom-tab-btn active px-6 py-3 text-xs font-bold uppercase tracking-widest text-slate-400 border-b-2 border-transparent hover:text-white transition-colors" onclick="window.switchBottomTab('historia')">História</button>
                    <button class="bottom-tab-btn px-6 py-3 text-xs font-bold uppercase tracking-widest text-slate-400 border-b-2 border-transparent hover:text-white transition-colors" onclick="window.switchBottomTab('anotacoes')">Anotações & Inventário</button>
                    <button class="bottom-tab-btn flex items-center gap-2 px-6 py-3 text-xs font-bold uppercase tracking-widest text-slate-400 border-b-2 border-transparent hover:text-white transition-colors" onclick="window.switchBottomTab('objetivos')">
                ...
                <div class="p-4 flex-grow flex flex-col">
                    ...
                    <div id="tab-bottom-objetivos" class="bottom-tab-content hidden h-full animate-fade-in overflow-y-auto custom-scroll">
                        <div id="objectives-root"></div>
                    </div>
                </div>
            </div>
            
            <div class="flex gap-4 pt-4 border-t border-slate-700 shrink-0 mt-auto">
                <button id="btn-voltar-painel" class="btn bg-slate-800 hover:bg-slate-700 text-slate-300 px-8 py-3 font-bold uppercase tracking-wider border border-slate-600 transition shadow">Voltar</button>
                <button id="btn-salvar-ficha" class="btn bg-amber-600 hover:bg-amber-500 text-black flex-grow py-3 font-bold uppercase tracking-widest shadow-lg transform transition active:scale-95"><i class="fas fa-save mr-2"></i> Salvar e Sincronizar Ficha</button>
            </div>
        </div>`;
...
function setupImageGallery(container, urls, mainKey) {
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        let classes = 'upload-slot w-full aspect-square relative rounded-md border-2 flex items-center justify-center cursor-pointer overflow-hidden transition-all group ';
        ...
        slot.innerHTML = `
            <button type="button" class="star-btn absolute top-1 right-1 w-6 h-6 rounded-full bg-black/60 flex items-center justify-center z-30 transition-all ${isMain ? 'text-amber-500 opacity-100' : 'text-slate-400 opacity-0 group-hover:opacity-100 hover:text-amber-400 hover:scale-110'}" title="Definir como Principal">
        ...
        input.addEventListener('change', e => {
            ...
            if (file) {
                ...
                slot.classList.add('border-slate-600');
...
window.addNewObjective = async function() {
    ...
    try {
        ...
        input.value = ''; // Limpa o input no sucesso
