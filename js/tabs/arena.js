...
// Constantes exatas do seu projeto
...
window.arena = {
    ...
    // --- INICIALIZAÇÃO BASEADA EM SESSÃO ---
    init: function() {
        if (this.unsub) { 
            this.unsub(); 
            this.unsub = null; 
        ...
        if (!activeSessionId || activeSessionId === 'world') {
            ...
            document.getElementById('arena-layer-grid').innerHTML = '';
            document.getElementById('arena-layer-tokens').innerHTML = '';
            document.getElementById('arena-layer-map').innerHTML = '';
            return;
        }
        ...
        this.unsub = onSnapshot(doc(db, "rpg_sessions", this.sessionDocId), (snap) => {
            ...
            if (logContainer) {
                ...
                if (logs.length > 0) {
                    ...
                    logContainer.innerHTML = '<div class="text-slate-600 italic p-2">Histórico de combate vazio...</div>';
                }
            }
            
            // --- ATUALIZAÇÃO VISUAL DO BOTÃO MODO LIVRE ---
    ...
    showError: function(text) {
        ...
        container.innerHTML = `<div id="arena-overlay-msg" class="absolute inset-0 flex items-center justify-center bg-slate-900/90 z-50 text-amber-500 font-cinzel text-xl font-bold p-10 text-center">${text}</div>`;
    },
    ...
    checkMasterRole: function(sessionData) {
        ...
        const btnAssume = document.getElementById('btn-assume-gm');
        const statusMsg = document.getElementById('arena-status-msg');

        if (this.isMaster) {
            if(container) {
                container.classList.add('is-master');
                container.classList.remove('is-player');
            }
            if(panel) panel.classList.remove('hidden');
            if(btnAssume) btnAssume.classList.add('hidden');
            ...
            if(container) {
                ...
                container.classList.remove('is-master');
            }
    ...
    // --- AÇÕES DO MESTRE ---
    assumeMaster: async function() {
        alert("O controle agora é definido pelo dono da sessão.");
    },
    ...
    modStat: async function(tokenId, field, multiplier, inputId) {
        ...
        if (t.type === 'player' && t.originId) {
            ...
            if (fichaData) {
                const cascade = window.calculateStatCascade(fichaData.ficha || fichaData, field, change);
                newTotal = cascade.total;
                
                Object.assign(fichaData.ficha || fichaData, cascade.updates);
        ...
        try {
            ...
            if (cascadeUpdates && charIdToUpdate) {
                try {
                    const sheetRef = doc(db, "rpg_fichas", charIdToUpdate);
                    await updateDoc(sheetRef, cascadeUpdates);
                } catch(e) {
                    console.warn(`Aviso: Falha ao salvar dano/cura na ficha raiz do jogador ${charIdToUpdate}, mas a Arena foi salva.`, e);
                }
            }
        } catch (err) {
            console.error("Erro sync:", err);
            alert("Erro ao salvar status na arena.");
        }
    },
    ...
    executeAreaSkill: async function(targetQ, targetR) {
        ...
        const debuffFome = window.getFomeDebuffMultiplier(ficha);
        ...
        try {
            ...
            if (myTokenId && charId && charData) {
                const cascade = window.calculateStatCascade(charData.ficha || charData, 'mp', -Math.abs(costToDeduct));
                cascadeUpdates = cascade.updates;
                Object.assign(charData.ficha || charData, cascade.updates);
                myToken.mp = cascade.total;
            }
            ...
            if (myTokenId && cascadeUpdates) {
                sessionUpdates[`arena_state.tokens.${myTokenId}.mp`] = myToken.mp;
            }
            ...
            if (cascadeUpdates && charId) {
                try {
                    ...
                    console.warn(`Erro ao descontar MP da ficha raiz (limite 1MB), mas descontado na Arena.`, e);
                }
            }

            if (myTokenId) this.showFloatingText(`-${costToDeduct} MP`, myTokenId, 'blue');
            hitTokensIds.forEach(id => this.showFloatingText(`HIT! ${totalDano}`, id, 'red'));
    ...
    spawnTokenAt: async function(q, r) {
        ...
        try {
                ...
                sessionData.playerIds.forEach(async (playerId) => {
                    if (playerId !== originId) {
                        ...
                        try {
                            ...
                            console.warn(`Aviso: Falha ao adicionar à coleção do jogador ${playerId} (Possível limite de 1MB).`);
                        }
                    }
                });
            }
            ...
            alert("Erro ao inserir token. Recarregue a página e tente novamente.");
        }
    },
    ...
    renderArenaSkills: function() {
        ...
        if (this.isMaster && this.selectedTokenId) {
            ...
            if (token && token.originId) {
                let cached = globalState.cache.mobs.get(token.originId) || globalState.cache.players.get(token.originId);
                if (cached) {
        ...
        if (favs.length === 0) {
            container.innerHTML = `<span class="text-[10px] text-slate-500 italic p-2">Nenhuma habilidade favorita encontrada para <strong>${actorName}</strong>.</span>`;
            return;
        }
    ...
    cancelTargeting: function() {
        this.targeting = null;
        document.getElementById('arena-skill-tray').classList.add('hidden');
        document.querySelectorAll('.arena-skill-btn').forEach(b => b.classList.remove('selected'));
    ...
    requestAttack: function(targetTokenId) {
        ...
        document.getElementById('modal-skill-cost').textContent = skill.mpCost;
    ...
    closeAttackModal: function() {
        document.getElementById('arena-attack-modal').classList.add('hidden');
        document.getElementById('arena-attack-modal').classList.remove('flex');
    },
    ...
    confirmAttack: async function(targetTokenId, targetName) {
        ...
        try {
            ...
            if (myTokenId && charId && charData) {
                const cascade = window.calculateStatCascade(charData.ficha || charData, 'mp', -costToDeduct);
                cascadeUpdates = cascade.updates;
                Object.assign(charData.ficha || charData, cascade.updates);
                myToken.mp = cascade.total;
            }
            ...
            this.showFloatingText(`-${costToDeduct} MP`, myTokenId, 'blue');
            this.showFloatingText(`HIT! ${totalDano}`, targetTokenId, 'red');
    ...
    showFloatingText: function(text, tokenId, color = 'white') {
        if(!tokenId) return;
        const token = this.data.tokens[tokenId];
        if(!token) return;

        const pos = this.hexToPixel(token.q, token.r);
        const container = document.getElementById('arena-viewport');
        
        const floatEl = document.createElement('div');
        floatEl.textContent = text;
        floatEl.className = `absolute font-cinzel font-bold text-shadow pointer-events-none animate-float-up z-50`;
        floatEl.style.left = `${pos.x}px`;
        floatEl.style.top = `${pos.y - 40}px`;
        floatEl.style.transform = "translateX(-50%)";
        floatEl.style.color = color === 'red' ? '#ef4444' : (color === 'blue' ? '#60a5fa' : '#ffffff');
        floatEl.style.textShadow = "0 0 4px black";
    ...
    renderTokens: function() {
        const tokensLayer = document.getElementById('arena-layer-tokens');
        ...
        if (this.selectedTokenId && tokens[this.selectedTokenId]) {
            const t = tokens[this.selectedTokenId];
            if (!this.isMaster || this.targeting) { 
                ...
                if(t.originCollection === 'rpg_fichas') {
                    ...
                    if(cached) {
                        ...
                        const weightStats = window.calculateWeightStats(cached, cached.levelPersonagemBase || 1);
                        realMove = Math.max(0, baseMove - weightStats.penalty);
                    }
                } else {
    ...
    updateActionHUD: function() {
        ...
        const currentTurnId = this.data.ordemIniciativa?.[this.data.turnoIndex];
        const isMyTurn = this.isMaster || (currentTurnId && currentTurnId.startsWith(globalState.selectedCharacterId));
        ...
        if(isMyTurn) {
            ...
            const updateBtn = (id, used) => {
                ...
                if(used) btn.classList.add('used'); else btn.classList.remove('used');
            };
    ...
    hexToPixel: function(q, r) { return { x: HEX_SIZE * (Math.sqrt(3) * q + Math.sqrt(3)/2 * r) + 60, y: HEX_SIZE * (3./2 * r) + 60 }; },
    getHexPoints: function(x, y) {
        let p = ""; for(let i=0; i<6; i++) { const rad = Math.PI/180*(60*i-30); p+=`${x+(HEX_SIZE-1)*Math.cos(rad)},${y+(HEX_SIZE-1)*Math.sin(rad)} `; } return p;
    },
    updateTransform: function() { document.getElementById('arena-svg').style.transform = `translate(${this.drag.panX}px, ${this.drag.panY}px) scale(${this.scale})`; },
    zoom: function(f) { this.scale *= f; this.updateTransform(); },
    setTool: function(t) { this.tool = t; this.spawnTarget = null; document.getElementById('arena-tool-display').textContent = `Ferramenta: ${t}`; },
    ...
    uploadMap: async function(inputElement) {
        ...
        label.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processando...';
        label.classList.add('opacity-50', 'pointer-events-none');

        try {
            // Usa o compressor de imagens global para otimização
            ...
            if (currentMapUrl && currentMapUrl.includes('firebasestorage')) {
                try {
                    ...
                    console.log("[Arena] Mapa antigo deletado do Firebase para economizar espaço.");
                } catch(err) {
                    console.warn("[Arena] O mapa antigo não pôde ser deletado (pode já não existir).", err);
                }
            }
            ...
            console.error("Erro no upload do mapa:", e);
